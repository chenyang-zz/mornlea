...
## PR 1 whole-change review

- Review baseline: `107e4db90b91d69e19e943ed4f8fb0c66209e6b3` to Worker head `3b73411bc9a7161743314601638c0bf718da6b95`. Independent read-only reviews split companion, chunk and domain-rule paths; the controller checked scope, source authority, task status and stage gates. Initial verdict was not ready: node 4.1 remained open after Linux-only `make dev-check`/`make test-race` failures, and the code had five important boundary gaps.
- Companion review found that a valid-count lifecycle summary was cloned before its 2,048-byte check. Commit `f48ac807` sorts bounded references, validates borrowed records/lifecycles/queues, then clones. The allocation-sensitive regression failed on the original 2,049-byte copy and now passes; focused companion bounds 7/7 and `runtime_contract companion_` 6/6 passed on the macOS review checkout.
- Chunk review found that older output could omit nondefault slots, current validation rejected raw pre-v5 multi-tool drops that the decoder admits, and an envelope encode converted each section twice. Domain review found a separate smelting whitelist in the furnace admission helper. Commit `3b2468ed` distinguishes raw historical logical admission from current-value envelope admission, rejects state omitted or changed by the target schema, uses a private validated appender, and delegates smelting to `mornlea_domain`. New tests failed on the original omission and legacy-drop paths; focused chunk aggregate 6/6 and `runtime_contract chunk_` 10/10 passed on the macOS review checkout. A second independent review found and then verified the fix for an inactive tool drop changed by v4 durability migration.
- The safety design, delta and task packet were reconciled to the reviewed behavior. The codec successor's chunk preflight/context packets were also tightened so a later writer does not duplicate safety validation or reject raw legacy logical reserialization. No online authority, Go production codec, fixture, save version, protocol version or ABI changed.
- Original Worker commits contain `Co-authored-by` footers despite the repository's one-line commit rule. The review-fix commits are one line. Existing commits were not rewritten because force push is prohibited without explicit authorization; this process discrepancy does not change the verified runtime behavior.
- Node 4.1 remains open until the required macOS full gates and strict OpenSpec validation run against the review result SHA. Architecture skill: no change.

## Node 4.1 macOS acceptance

- Accepted code and planning baseline before the evidence commit: `e05262b194db46e6a2f07a17045b72f24f9b822f`; pre-safety base: `107e4db90b91d69e19e943ed4f8fb0c66209e6b3`. The earlier Linux gate failures were platform-specific: this macOS checkout contains the Darwin client files and completed the same full commands.
- `make rust`: exit 0. `rustup run 1.97.1 cargo fmt --manifest-path packages/engine/Cargo.toml --all --check`: exit 0. `cargo test -p mornlea_storage --tests --locked -- --list` discovered 104 tests: 19 library, 68 runtime contract, 6 chunk aggregate, 7 companion bounds, 3 domain values and 1 legacy queue. `cargo test -p mornlea_storage --tests --locked`: 104/104 passed.
- `make rust-check`: exit 0, including workspace format, warning-denying Clippy and workspace tests. `make dev-check`: exit 0 across all six Go modules and the Rust checks. `make test-race`: exit 0 across all six Go modules, including the Darwin client, runtime oracle and audit packages. `go test ./packages/audit -count=1`: exit 0.
- `openspec validate --all --strict --no-interactive`: 128 passed, 0 failed. `git diff --check 107e4db90b91d69e19e943ed4f8fb0c66209e6b3..HEAD`: exit 0. `git diff --exit-code 107e4db90b91d69e19e943ed4f8fb0c66209e6b3..HEAD -- testdata/runtime-migration packages/server/storage`: exit 0; the existing corpus and production Go storage code are unchanged.
- Independent review covered companion admission, historical/current chunk encoding and domain rule ownership. Review fixes were independently re-reviewed with an accept verdict; the controller inspected the integrated diff and ran the acceptance gates above. Existing v1–v9/current fixtures and downstream consumers passed under the full storage target. No live data, save schema, protocol or ABI changed. Node 4.1 is complete and the codec successor may use this accepted baseline.
- Rollback is a scoped revert of safety implementation/review commits back to the pre-safety base; there is no live-data migration to reverse. Architecture skill: no change; the new behavior belongs in the canonical storage capability specification, with no additional stable cross-task working rule.
