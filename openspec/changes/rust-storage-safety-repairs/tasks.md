...
- [ ] 2.1 [Apply aggregate validity to current chunk encode](plans/01-admission.md#node-2-1). Modify `src/chunk.rs`; run `rustup run 1.97.1 cargo test --manifest-path packages/engine/Cargo.toml -p mornlea_storage --test safety_chunk_aggregate --locked current_`.
